...
function UILibrary:ToggleUI()
	...
	if self.Window then
		...
		if targetVisible then
			...
			self.IsToggling = true
			-- Save current size before hiding (only if not maximized and size is valid)
			if not self.IsMaximized and self.Window.Size.X.Offset >= CONFIG.MinWindowSize.X then
				self:SaveWindowSize(self.Window.Size)
				self.SavedWindowSize = self.Window.Size
			end
...
function UILibrary:SaveWindowSize(size)
	...
	if size.X.Offset < CONFIG.MinWindowSize.X or size.Y.Offset < CONFIG.MinWindowSize.Y then
		warn("[nozomi] attempted to save invalid window size:", size.X.Offset, size.Y.Offset)
		return
	end
